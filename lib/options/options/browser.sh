...
if [ -z "$optionalSelect" ] ;then
    ...
    if [ $optionalSelect == "brave-bin" ]; then
        echo 'brave' > "$HOME/.config/main/settings/browser.sh"
    elif [ $optionalSelect == "brave-browser" ]; then
        echo 'brave' > "$HOME/.config/main/settings/browser.sh"
    elif [ $optionalSelect == "zen-browser-bin" ]; then
        echo 'zen-browser' > "$HOME/.config/main/settings/browser.sh"
    else
        echo "$optionalSelect" > "$HOME/.config/main/settings/browser.sh"
    fi
    _selectCategory
fi