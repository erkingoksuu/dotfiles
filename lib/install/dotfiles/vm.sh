...
_install_vm() {
    if grep -Fxq "kvm.conf" $temp_directory/$version/.config/hypr/conf/environment.conf
    then
        echo ":: KVM Environment already set."
    else
        if gum confirm "Do you want to install the KVM environment variables?"; then
            echo "source = ~/.config/hypr/conf/environments/kvm.conf" >  $temp_directory/$version/.config/hypr/conf/environment.conf
